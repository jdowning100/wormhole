...
contract Bridge is BridgeGovernance, ReentrancyGuard {
    ...
    function _createWrapped(BridgeStructs.AssetMeta memory meta, uint64 sequence) internal returns (address token) {
        ...
        bytes32 startingSalt = keccak256(abi.encodePacked(meta.tokenChain, meta.tokenAddress));
        bytes32 salt = findSaltForAddress(keccak256(type(BridgeToken).creationCode), startingSalt);
